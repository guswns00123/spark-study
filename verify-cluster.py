...
# ----------------------------------------------------------------
# 6. 파일 I/O 테스트 (선택 — Windows Docker Desktop에서 종종 실패)
#    Hadoop이 깊은 _temporary 디렉토리를 동시에 mkdir할 때 가상화 레이어가 실패시킴.
#    학습에는 무관 — 인메모리 DataFrame + Jupyter로 99% 커버됨.
# ----------------------------------------------------------------
print("\n[TEST 4] 파일 I/O 테스트 (선택)")
...
try:
    ...
    print("  [PASS] 파일 I/O 성공")
except Exception as e:
    msg = str(e).split('\n')[0][:120]
    print(f"  [SKIP] Windows Docker 환경 mkdir 이슈로 미지원")
    print(f"         {msg}...")
    print(f"         → 학습 진행에는 영향 없음")
